function polynomial_w_plot(data, degree, point, slope)
  % Load data 
  x = data(:,1);
  y = data(:,2);
  n = length(x);
  ...
  % Predict the point and plot it
  y_point = 0;
  for i = 1:degree+1
    y_point = y_point + B(i)*(point.^(i-1));
  end
  scatter(point, y_point, 'filled', 'MarkerFaceColor', 'green');
  hold off;

  % Label the graph
  title_label = sprintf('Polynomial Fit Degree %f', degree);
  legend('Data', 'Polynomial Fit', 'Point p');
  title(title_label);
  xlabel('x');
  ylabel('y');
